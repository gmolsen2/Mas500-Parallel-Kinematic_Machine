...
Separationx_posE = zeros(n_i,n_j);
Separationx_posW = zeros(n_i,n_j);
Separationx_negE = zeros(n_i,n_j);
Separationx_negW = zeros(n_i,n_j);
Separationy_posE = zeros(n_i,n_j);
Separationy_posW = zeros(n_i,n_j);
Separationy_negE = zeros(n_i,n_j);
Separationy_negW = zeros(n_i,n_j);
...
for i=1:n_i
    ...
    for j=1:n_j
        ...
        while py < 90 && isreal(A2(1)+B2(1))
            ...
            Separation_E = D2_x - C2_x;
            Separation_W = E2_x - F2_x;
           ...
           %Saving relevant results
            if  isreal(A2(1)+B2(1)+Separation_E+Separation_W)
                Resultsy_pos(i,j) = py;
                Separationy_posE(i,j) = Separation_E * 1E3;
                Separationy_posW(i,j) = Separation_W * 1E3;
            end
        end
           ...
           %Saving relevant results
            ...
            elseif d_PA < d/2 & d_PB < d/2 && d_P1A < d_P & d_P2B < d_P
                break
                   ...
                   Resultsy_neg(i,j) = -py;
                   Separationy_negE(i,j) = Separation_E * 1E3;
                   Separationy_negW(i,j) = Separation_W * 1E3;
            end
        end
        ...
        while px < 90 && breaksignal == boolean(0)
            ...
            if ~isreal(pz)
                break
            elseif ~isreal(C2_x+D2_x+E2_x+F2_x)
                break
            end
            ...
            if breaksignal == boolean(0)
                Resultsx_pos(i,j) = px;
                Separationx_posE(i,j) = Separation_E * 1E3;
                Separationx_posW(i,j) = Separation_W * 1E3;
            end
        ...
        while px > -90 && breaksignal == boolean(0)
            ...
            if breaksignal == boolean(0)
                Resultsx_neg(i,j) = -px;
                Separationx_negE(i,j) = Separation_E * 1E3;
                Separationx_negW(i,j) = Separation_W * 1E3;
            end
...
subplot(3,1,2)
surf(AxesY,AxesZ,Separationy_posE)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
...
subplot(3,1,3)
surf(AxesY,AxesZ,Separationy_posW)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
...
subplot(3,1,2)
surf(AxesY,AxesZ,Separationy_negE)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
...
subplot(3,1,3)
surf(AxesY,AxesZ,Separationy_negW)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
...
subplot(3,1,2)
surf(AxesY,AxesZ,Separationx_posE)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
zlabel('Sled separation E [mm]')
title('Maximum Separation E')

subplot(3,1,3)
surf(AxesY,AxesZ,Separationx_posW)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
zlabel('Sled separation W [mm]')
title('Maximum Separation W')
...
subplot(3,1,2)
surf(AxesY,AxesZ,Separationx_negE)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
...
subplot(3,1,3)
surf(AxesY,AxesZ,Separationx_negW)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
