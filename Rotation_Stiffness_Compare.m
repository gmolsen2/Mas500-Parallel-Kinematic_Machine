...
%Test conditions
n_i = 100; %number of discretizations for z-direction
n_j = 100; %number of discretizations for y-direction
...
for i=1:n_i
    ...
    for j=1:n_j
        ...
        while py < 90 && (limit*testx < k_x | limit*testy < k_y | limit*testz < k_z) && isreal(CoR_Rx+CoR_Lx)
            py=py + 0.2; %[deg]
            A = A_rot(0,py,0);
            %Calculations
            %Finding global position of ball socket joint on tool platform
            ...
            if limit*testx < k_x && isreal(A2(1)+B2(1)+CoR_Rx+CoR_Lx)
                Resultsx(i,j) = py;
            end

            if limit*testy < k_y && isreal(A2(1)+B2(1)+CoR_Rx+CoR_Lx)
                Resultsy(i,j) = py;
            end

            if limit*testz < k_z && isreal(A2(1)+B2(1)+CoR_Rx+CoR_Lx)
                Resultsz(i,j) = py;
            end
        end
