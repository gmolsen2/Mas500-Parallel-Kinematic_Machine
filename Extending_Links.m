...
Extensiony_posC = zeros(n_i,n_j);
Extensiony_posF = zeros(n_i,n_j);
Extensiony_negC = zeros(n_i,n_j);
Extensiony_negF = zeros(n_i,n_j);
    ...
    for j=1:n_j
        ...
        while py < 90 && isreal(A2(1)+B2(1))
           ...
           %Saving relevant results
            if  isreal(A2(1)+B2(1))
                Resultsy_pos(i,j) = py;
                Extensiony_posC(i,j) = Extention_C * 1E3;
                Extensiony_posF(i,j) = Extention_F * 1E3;
            end
        end
           ...
           %Saving relevant results
            ...
            elseif d_PA < d/2 & d_PB < d/2 && d_P1A < d_P & d_P2B < d_P
                break
                   ...
                   Resultsy_neg(i,j) = -py;
                   Extensiony_negC(i,j) = Extention_C * 1E3;
                   Extensiony_negF(i,j) = Extention_F * 1E3;
            end
        end
        ...
        while px < 90 && breaksignal == boolean(0)
            ...
            while error > 1E-2 && count < 1000 && breaksignal == boolean(0)
                A = A_rot(px,0,pz);
                A1 = TCP + A*A1m;
                B1 = TCP + A*B1m;
...
subplot(3,1,2)
surf(AxesY,AxesZ,Extensiony_posC)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
...
subplot(3,1,3)
surf(AxesY,AxesZ,Extensiony_posF)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
...
subplot(3,1,2)
surf(AxesY,AxesZ,Extensiony_negC)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
...
subplot(3,1,3)
surf(AxesY,AxesZ,Extensiony_negF)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
