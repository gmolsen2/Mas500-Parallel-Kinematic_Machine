...
ExtensionC = zeros(n_i,n_j);
ExtensionF = zeros(n_i,n_j);
...
for i=1:n_i
    ...
    for j=1:n_j
        ...
        if  isreal(A2(1)+B2(1)+C2(1)+D2(1)+E2(1)+F2(1))
            Resultsx(i,j) = k_x;
            Resultsy(i,j) = k_y;
            Resultsz(i,j) = k_z;
            SingularityCheck(i,j) = cond(H);
            ExtensionC(i,j) = Extention_C * 1E3;
            ExtensionF(i,j) = Extention_F * 1E3;
        end
    end
end
...
figure(5)
surf(AxesY,AxesZ,ExtensionC)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
zlabel('Joint position C-Link [mm]')
title('Extension of joint C')

figure(6)
surf(AxesY,AxesZ,ExtensionF)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
zlabel('Joint position F-Link [mm]')
title('Extension of joint F')
