...
JointAngleD = zeros(n_i,n_j);
JointAngleE = zeros(n_i,n_j);
...
for i=1:n_i
    ...
    for j=1:n_j
        ...
        if isreal(Angle_D+Angle_E)
        Resultsx(i,j) = k_x;
        Resultsy(i,j) = k_y;
        Resultsz(i,j) = k_z;
        SingularityCheck(i,j) = cond(H);
        JointAngleD(i,j) = alphaD;
        JointAngleE(i,j) = alphaE;
        end
    end
end
...
figure(5)
surf(AxesY,AxesZ,JointAngleD)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
zlabel('Joint position D [deg]')
title('Joint position of joint D')

figure(6)
surf(AxesY,AxesZ,JointAngleE)
grid on
colorbar
xlabel('Y-axis [mm]')
ylabel('Z-axis [mm]')
zlabel('Joint position E [deg]')
title('Joint position of joint E')
